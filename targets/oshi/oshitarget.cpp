...
OshiTarget::OshiTarget(bool useSSL, const std::string& url, const std::string& name): HttplibTarget(useSSL,url,name){
...
void OshiTarget::uploadFile(BackendRequirements requirements, const File& file, std::function<void(std::string)> successCallback, std::function<void(std::string)> errorCallback){
  
  httplib::MultipartFormDataItems items = {
    { "f", file.getContent(), file.getName(), file.getMimetype() }
  };
