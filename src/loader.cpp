...
std::filesystem::path loadPath(const std::string& filePath) {
  ...
  switch(fileStatus.type()) {
    ...
    case std::filesystem::file_type::symlink:
      logger.log(Logger::Fatal) << "You tried to upload the file " << filePath
                                << " , which is a symlink. Those are currently not supported." << '\n';
      quit::failedReadingFiles();
    case std::filesystem::file_type::block:
      logger.log(Logger::Fatal) << "You tried to upload the blockdevice " << filePath
                                << " . Currently only regular files and directories are supported." << '\n';
      quit::failedReadingFiles();
    case std::filesystem::file_type::character:
      logger.log(Logger::Fatal) << "You tried to upload the characterdevice " << filePath
                                << " . Currently only regular files and directories are supported." << '\n';
      quit::failedReadingFiles();
    case std::filesystem::file_type::fifo:
      logger.log(Logger::Fatal) << "You tried to upload the pipe "
                                << filePath << " . Currently only regular files and directories are supported. Pipes may be supported somewhere in the future."
                                << '\n';
      quit::failedReadingFiles();
    case std::filesystem::file_type::socket:
      logger.log(Logger::Fatal) << "You tried to upload the socket " << filePath
                                << " . Currently only regular files and directories are supported." << '\n';
      quit::failedReadingFiles();
    case std::filesystem::file_type::unknown:
      logger.log(Logger::Fatal) << "Failed to determine to filetype of " << filePath
      ...
      quit::failedReadingFiles();
    default:
      logger.log(Logger::Fatal) << "You tried to upload a strange filetype " << filePath
                                << " . I do not know how you got here, but you probably know what you did wrong." << '\n';
      quit::failedReadingFiles();
    ...
    case std::filesystem::file_type::directory:
      ...
      if(accessResult != 0) {
        logger.log(Logger::Fatal) << "You do not have the permission to access " << filePath
