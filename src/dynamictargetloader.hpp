...
std::vector<std::shared_ptr<Target>> loadTargetsFromFile(const std::filesystem::path& file) {
  ...
  if(errorMessage != NULL) {
    ...
    logger.log(Logger::Debug) << "Library " << file << " does not contain 'TargetList load_targets_dynamically()'. Check, if " << file
                              << " really is an upload target. If you build the library yourself you can use the setTargetType() macro to "
                                 "generate 'TargetList load_targets_dynamically()' for you. "
                              << '\n';
